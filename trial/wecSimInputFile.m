...
simu.simMechanicsFile = 'testModel.slx';      % Specify Simulink Model File
...
waves.height = 0.02;                     % Wave Height [m]
waves.period = 1;                       % Wave Period [s]
waves.waterDepth = 0.508;               % 20 in
...
body(1).geometryFile = 'geometry/orientedfloat.stl';    % Location of Geomtry File
body(1).mass = 'equilibrium';                   
    % Body Mass. The 'equilibrium' Option Sets it to the Displaced Water 
    % Weight.
body(1).inertia = [81371 33457 81371];  % Moment of Inertia [kg*m^2]     
body(1).volume = 0.0068;
body(1).centerGravity = [0, 0, 0.0382025];
body(1).centerBuoyancy = [-0.12319, 0.2286, -0.0547116] - [-0.107696, 0.2286, 0];

%% PTO and Constraint Parameters
% Floating (3DOF) Joint
constraint(1) = constraintClass('Constraint1'); % Initialize Constraint Class for Constraint1
constraint(1).location = [0 0 0.75];               % Constraint Location [m]

% Translational PTO
pto(1) = ptoClass('PTO1');                      % Initialize PTO Class for PTO1
pto(1).stiffness = 0;                           % PTO Stiffness [N/m]
pto(1).damping = 10;                       % PTO Damping [N/(m/s)]
pto(1).location = [0 0 0.75];                      % PTO Location [m]
